  ...
  try {
    ...
    try {
      ...
      await connection.query<ResultSetHeader>(
        `UPDATE novel SET title = ?, description = ?, coverUrl = ?, posterUrl = ?, author = ?, status = ?, updatedAt = NOW() WHERE id = ?`,
        [title, description || '', coverUrl || null, posterUrl || null, author, status || 'ONGOING', novelId]
      );
