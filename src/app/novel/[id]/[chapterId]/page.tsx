...
async function getChapterMeta(novelId: string, chapterNumber: string) {
  try {
    ...
    const [chapters] = await pool.query<RowDataPacket[]>(
      `SELECT c.id, c.chapterNumber, c.title, c.isLocked, c.createdAt,
              n.title AS novelTitle, n.author, n.coverUrl
       FROM chapter c
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  const url = `${SITE_URL}/novel/${id}/${chapterId}`;
      ...
      images: [
        {
          ...
          width: 400,
          height: 600,
          alt: `Ảnh bìa ${chapter.novelTitle}`,
        },
      ],
...
export default async function ReadingPage({ params }: Props) {
  ...
  const articleJsonLd = chapter
    ? {
        ...
        url: `${SITE_URL}/novel/${id}/${chapterId}`,
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      },
      {
        ...
        item: `${SITE_URL}/novel/${id}`,
      },
      {
        "@type": "ListItem",
        position: 3,
        name: chapterTitle,
        item: `${SITE_URL}/novel/${id}/${chapterId}`,
      },
    ],
  };
