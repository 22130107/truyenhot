...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  try {
    ...
    const [novels] = await pool.query<RowDataPacket[]>(
      `SELECT id, updatedAt FROM novel ORDER BY updatedAt DESC LIMIT 5000`
    );
    ...
    const [chapters] = await pool.query<RowDataPacket[]>(
      `SELECT c.novelId, c.chapterNumber, c.createdAt
       FROM chapter c
    ...
    const chapterPages: MetadataRoute.Sitemap = chapters.map((chapter) => ({
      url: `${SITE_URL}/novel/${chapter.novelId}/${chapter.chapterNumber}`,
