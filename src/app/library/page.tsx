...
export default function LibraryPage() {
  ...
  return (
    <div className="min-h-screen bg-[rgb(31,38,25)] text-white flex flex-col" style={{ fontFamily: "Figtree, ui-sans-serif, system-ui, sans-serif" }}>
      <Header />
      ...
      <main className="max-w-7xl mx-auto px-4 md:px-8 pt-28 pb-20 flex-1 w-full">
        ...
        <div className="mb-10">
          ...
          <p className="text-gray-400">Quản lý bộ sưu tập của bạn và theo dõi tiến trình đọc</p>
        </div>

        {/* Tabs */}
        <div className="flex flex-wrap gap-6 mb-10 border-b border-neutral-800/50 pb-0">
          {TABS.map((tab) => (
            <button
              key={tab.key}
            ...
            >
              {tab.label}
              {activeTab === tab.key && (
                <div className="absolute bottom-0 left-0 right-0 h-0.5 bg-[rgb(250,204,21)] shadow-[0_0_10px_rgba(250,204,21,0.5)]" />
              )}
            </button>
          ))}
        </div>
        ...
        {loading ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {Array.from({ length: 6 }).map((_, i) => (
              <div key={i} className="bg-neutral-800/40 rounded-xl overflow-hidden animate-pulse">
          ...
          <div className="flex flex-col items-center justify-center py-24 text-center">
            ...
            <p className="text-neutral-400 text-lg font-medium mb-2">Chưa có truyện nào</p>
            <p className="text-neutral-500 text-sm mb-6">Hãy bắt đầu đọc để thêm truyện vào thư viện</p>
            <Link href="/browse" className="bg-[rgb(250,204,21)] text-black font-bold px-6 py-2.5 rounded-xl hover:bg-yellow-300 transition-colors text-sm">
              Khám phá truyện
            </Link>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {books.map((book) => {
              ...
              return (
                <div key={book.id} className="group bg-[rgb(10,10,10)] border border-neutral-800 rounded-xl overflow-hidden hover:border-neutral-600 transition-all hover:shadow-2xl">
                  {/* Cover */}
                  <Link href={`/novel/${book.id}/${continueChapter}`}>
                      ...
                      {progress !== null && (
                        <div className="absolute bottom-0 left-0 right-0 p-3">
                          <div className="flex justify-between items-center mb-1.5">
                            ...
                            <span className="text-[10px] font-bold text-[rgb(250,204,21)]">{progress}%</span>
                          </div>
                          <div className="w-full h-1 bg-white/20 rounded-full overflow-hidden">
                            <div
                              className="h-full bg-[rgb(250,204,21)] shadow-[0_0_8px_rgba(250,204,21,0.6)] transition-all"
                  ...
                  <div className="p-4">
                    <Link href={`/novel/${book.id}`}>
                      <h3 className="font-bold text-[15px] mb-1 line-clamp-1 hover:text-[rgb(250,204,21)] transition-colors">{book.title}</h3>
                    </Link>
                    <p className="text-sm text-neutral-400 mb-3 truncate">Tác giả: {book.author}</p>
                      ...
                      {(book.libStatus === "READING" || book.libStatus === "COMPLETED" || book.libStatus === "SAVED" || book.libStatus === "LIKED") && (
                        <div className="flex items-center gap-1">
                          ...
                          <select
                            ...
                            className="text-[11px] bg-neutral-800 border border-neutral-700 text-neutral-300 rounded-lg px-2 py-1 cursor-pointer hover:border-neutral-500 transition-colors"
                          >
                    ...
                    {book.lastChapter && (
                      <Link href={`/novel/${book.id}/${continueChapter}`}>
                        <button className="mt-3 w-full flex items-center justify-center gap-2 bg-[rgb(250,204,21)]/10 hover:bg-[rgb(250,204,21)]/20 border border-[rgb(250,204,21)]/20 hover:border-[rgb(250,204,21)]/40 text-[rgb(250,204,21)] text-[12px] font-semibold py-2 rounded-lg transition-all">
                          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-4 h-4">
