...
export default function AdminEditChapterPage() {
  ...
  useEffect(() => {
    fetch(`/api/admin/novels/${novelId}/chapters/${chapterId}`)
      .then((r) => r.json())
      .then((data) => {
  ...
  const handleSave = async () => {
    if (!content || chapterNumber === "") {
      alert("Vui lòng nhập đầy đủ Số chương và Nội dung!");
      return;
    }
    setIsSaving(true);
    try {
      const res = await fetch(`/api/admin/novels/${novelId}/chapters/${chapterId}`, {
        method: "PUT",
