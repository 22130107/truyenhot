    ...
    <div className="flex gap-0.5">
      {[1, 2, 3, 4, 5].map((s) => (
        <svg
          ...
          viewBox="0 0 24 24"
          fill={s <= score ? "rgb(250,204,21)" : "none"}
          stroke={s <= score ? "rgb(250,204,21)" : "currentColor"}
          strokeWidth="2"
          className={`w-4 h-4 ${s <= score ? "" : "text-gray-600"}`}
        >
          ...
          {distribution.map((item, index) => (
            <div key={index} className={`items-center flex ${index > 0 ? "mt-[8px]" : ""}`}>
              ...
              <div className="grow overflow-hidden relative h-2 ml-[8px] bg-[rgb(55,_65,_81)] basis-[0%] rounded-[624.9375rem]">
                <div className="absolute h-2 left-0 top-0 bg-[rgb(250,_204,_21)] rounded-[624.9375rem]" style={{ width: `${item.percentage}%` }} />
              </div>
