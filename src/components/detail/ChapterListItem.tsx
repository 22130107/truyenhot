...
export function ChapterListItem({
  ...
  return (
    <div>
      <Link href={url}>
        <div className="items-center flex justify-between p-4 group hover:bg-white/5 transition-colors cursor-pointer">
          <div className="flex-1 min-w-0">
            <h6 className={`font-medium group-hover:text-[rgb(250,204,21)] transition-colors truncate ${locked ? "text-neutral-400" : ""}`}>
              Chương {number}{title ? `: ${title}` : ""}
            </h6>
            <p className="mt-1 text-[rgb(163,163,163)] text-[12px] leading-[16px]">{date}</p>
          </div>
      ...
      {showDivider && <div role="none" className="h-px w-full bg-neutral-800" />}
    </div>
  );
}
